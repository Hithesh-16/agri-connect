...
export async function getCotton(_req: AuthRequest, res: Response) {
  sendSuccess(res, {
    exchangePrices: {
...
export async function getEnam(_req: AuthRequest, res: Response) {
  sendSuccess(res, {
    overview: {
    ...
    registrationSteps: [
      { step: 1, title: 'Visit eNAM Portal or App', description: 'Go to enam.gov.in or download the eNAM mobile app from Play Store/App Store.', documents: [] },
      { step: 2, title: 'Select Registration Type', description: 'Choose your role: Farmer, Trader, Commission Agent (CA), or FPO.', documents: [] },
      { step: 3, title: 'Fill Registration Form', description: 'Enter personal details, address, bank account information, and select your nearest APMC mandi.', documents: ['Aadhaar Card', 'Bank Passbook/Cancelled Cheque', 'Passport Photo'] },
      { step: 4, title: 'Submit KYC Documents', description: 'Upload or physically submit required identity and address proof documents.', documents: ['Aadhaar Card', 'PAN Card (for traders)', 'Land Records / Pattadar Passbook (for farmers)', 'Trade License (for traders)'] },
      { step: 5, title: 'Verification & Approval', description: 'APMC/Mandi Secretary verifies documents and approves registration. You will receive login credentials via SMS.', documents: [] },
      { step: 6, title: 'Start Trading', description: 'Login to eNAM, create lots for sale, and participate in online bidding across connected mandis.', documents: [] },
    ],
    benefits: [
...
export async function getFinance(_req: AuthRequest, res: Response) {
  sendSuccess(res, {
    rxil: {
